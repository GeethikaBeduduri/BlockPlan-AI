...
def map_tdms_record(record: dict) -> dict:
    source_id = str(record["task_id"])
    return NormalizedTask(
        task_id=f"TDMS-{source_id}",
        source_system="TDMS",
        source_record_id=source_id,
        department="Traction",
        asset_id=str(record.get("asset_id", f"OHE-{source_id}")),
        corridor_id=str(record["corridor_id"]),
        defect_type=str(record.get("defect_type", "OHE_DEFECT")),
